...
function generaColGrafico(label: string, data: Array<{ x: number; y: number }>, key: number) {
	return (
		<div key={key} className="col-xl-4">
			...
			{<ScatterChart data={data}>{label}</ScatterChart>}
		</div>
	);
}
function generaColGraficoTotale(datasets: Array<Dataset>, key: number) {
	const totale = datasets.find(dataset => dataset.label === "Totale");
	return (
		<div key={key} className="col-xl-4">
			{totale !== undefined ? (
				<table className="table">
			...
			{<ScatterChartMulti datasets={datasets}></ScatterChartMulti>}
		</div>
	);
}
